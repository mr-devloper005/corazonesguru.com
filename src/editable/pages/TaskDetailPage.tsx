      ...
      <section className="mx-auto grid max-w-[1040px] gap-10 px-4 py-10 sm:px-6 lg:grid-cols-[minmax(0,680px)_274px] lg:px-8 lg:py-12">
        <article className="min-w-0">
          <div className="flex flex-wrap items-center gap-2 text-[10px] font-black uppercase tracking-[0.14em] text-[#6f35c8]">
            ...
            <span>/</span>
            <span>{categoryOf(post, 'Article')}</span>
          </div>
          ...
          <div className="mt-5 flex flex-wrap items-center justify-between gap-4 border-b border-[#d7d7d7] pb-4">
            ...
            <div className="flex items-center gap-3 text-[#a5a5a5]" aria-label="Share this article">
              {['X', 'R', 'F', 'G'].map((item) => (
                <span key={item} className="flex h-5 w-5 items-center justify-center text-sm font-black">{item}</span>
              ))}
            </div>
          </div>
          ...
          {images[0] ? (
            <figure className="mt-5">
              ...
              <figcaption className="mt-2 text-[11px] font-medium text-[#8a95a0]">Image: {categoryOf(post, 'Article')}</figcaption>
            </figure>
          ) : null}
          ...
          <section className="mt-8 bg-[#f5f5f5] p-5">
            ...
            <ul className="ml-4 list-disc space-y-2 text-base leading-7 marker:text-[#7e2fd3]">
          ...
          {images.slice(1, 2).map((image, index) => (
            <figure key={`${image}-${index}`} className="mt-10 border-y border-[#d7d7d7] py-6">
              <img src={image} alt="" className="mx-auto max-h-[420px] w-full object-contain" />
            </figure>
          ))}

          {related.length ? (
            <section className="mt-10 border-t border-[#d7d7d7] pt-5">
              <h2 className="text-xl font-black uppercase tracking-normal text-[#111] [font-family:Arial_Narrow,Arial,sans-serif]">You might also like_</h2>
              <div className="mt-4 grid gap-2">
                {related.slice(0, 4).map((item) => (
                  <Link key={item.id || item.slug} href={buildPostUrl('article', item.slug)} className="text-sm leading-6 text-[#182534] hover:text-[#7e2fd3]">
                    - {item.title}
                  </Link>
                ))}
              </div>
          ...
          <section className="mt-8 bg-[#def8ef] p-5">
            <h2 className="text-lg font-black uppercase tracking-normal text-[#1e2a34] [font-family:Arial_Narrow,Arial,sans-serif]">Our editorial process</h2>
            <p className="mt-3 text-sm leading-7 text-[#52616e]">
              At {slot4BrandConfig.siteName}, every article layout is designed to keep the claim, context, and next read easy to find. Editors shape headlines, summaries, and related links so readers can move through the archive with confidence.
            </p>
          </section>
  ...
  return (
    <aside className="min-w-0 space-y-9 lg:sticky lg:top-24 lg:self-start">
      <section className="border border-[#d2d2d2] bg-white">
        <div className="flex h-4 items-center justify-end gap-1 border-b border-[#c7d9d1] bg-[#e1fbf1] px-2">
          <span className="h-1.5 w-1.5 bg-[#52616e]" />
          <span className="h-1.5 w-1.5 bg-[#52616e]" />
          <span className="h-1.5 w-1.5 bg-[#52616e]" />
        </div>
        <div className="p-5">
          ...
          <form action="/signup" className="mt-4 flex overflow-hidden rounded-md border border-[#1c1c1c]">
            <input name="email" type="email" placeholder="Enter your email" className="min-w-0 flex-1 px-3 py-3 text-sm outline-none" />
            <button className="bg-black px-4 text-sm font-black text-white">Subscribe</button>
          </form>
        </div>
      </section>
      ...
      {related.length ? (
        <section>
          <h2 className="text-xl font-black uppercase tracking-normal [font-family:Arial_Narrow,Arial,sans-serif]">Latest lists_</h2>
          <div className="mt-4 grid gap-3">
            {related.slice(0, 3).map((item) => {
              const image = getImages(item)[0]
              return (
                <Link key={item.id || item.slug} href={buildPostUrl('article', item.slug)} className="grid grid-cols-[1fr_96px] border border-[#d2d2d2] bg-white">
                  <div className="min-w-0 p-3">
                    <h3 className="line-clamp-3 text-sm font-bold leading-5 text-[#1c2a36]">{item.title}</h3>
                    <p className="mt-2 text-[10px] font-black uppercase tracking-[0.14em] text-[#7b8791]">Article</p>
                  </div>
                  {image ? <img src={image} alt="" className="h-full min-h-24 w-24 object-cover" /> : <div className="flex min-h-24 w-24 items-center justify-center bg-[#f5f5f5]"><FileText className="h-6 w-6 text-[#9aa3aa]" /></div>}
                </Link>
              )
            })}
          </div>
      ...
      <section className="border border-[#d2d2d2] bg-white">
        ...
        <div className="p-5">
          <h2 className="flex items-center gap-2 text-lg font-black uppercase tracking-normal [font-family:Arial_Narrow,Arial,sans-serif]"><Rss className="h-4 w-4" /> Why trust {slot4BrandConfig.siteName}</h2>
          <p className="mt-4 text-sm leading-7 text-[#586777]">Our article pages are built for human-readable context, transparent related links, and concise summaries before deep reading.</p>
          <Link href="/about" className="mt-4 inline-flex text-sm font-bold text-[#7e2fd3] underline underline-offset-4">Learn about our process</Link>
        </div>
      </section>
    </aside>
  )
...
function BodyContent({ post, compact = false, articleStyle = false }: { post: SitePost; compact?: boolean; articleStyle?: boolean }) {
  if (articleStyle) {
    return <div className="article-content article-content--reference mt-8 max-w-none text-base leading-8 text-[#182534]" dangerouslySetInnerHTML={{ __html: formatPlainText(getBody(post)) }} />
  }
